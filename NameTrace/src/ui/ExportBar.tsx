  ...
  return (
    <section aria-labelledby="nt-export-title">
      <h2 id="nt-export-title" className="mb-2 text-sm font-medium text-ink">
        Export results {hits.length > 0 && <span className="font-normal text-muted">({hits.length.toLocaleString('en-ZA')})</span>}
      </h2>
      <div className="grid grid-cols-3 gap-2">
        <button type="button" className={btn} disabled={disabled || !!busy} onClick={() => run('csv')}>
          {busy === 'csv' ? <Spinner /> : <FileSpreadsheet aria-hidden className="size-4" />}
          CSV
        </button>
        <button type="button" className={btn} disabled={disabled || !!busy} onClick={() => run('pdf')}>
          {busy === 'pdf' ? <Spinner /> : <FileDown aria-hidden className="size-4" />}
          PDF
        </button>
        <button type="button" className={btn} disabled={disabled || !!busy} onClick={() => run('copy')}>
